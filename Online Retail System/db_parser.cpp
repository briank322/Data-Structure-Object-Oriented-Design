    ...
    while(!ifile.fail() && !error_) {
        ...
        if(state == FIND_SECTION) {
            ...
            if((line.size() > 3) && line[0] == '<'
             && line[1] == '/' && line[line.size()-1] == '>') {
                map<string, SectionParser*>::iterator it = 
                parsers_.find(sectionName);
                if(it != parsers_.end()) {
                    error_ = 
                    it->second->parse(sectionText,ds, startLineNo, errorMsg_);
                    if(error_) {
                        lineno_ = startLineNo;
                    }
        ...
        if(error_) {
            cerr << "Parse error on line " << lineno_ << 
            ": " << errorMsg_ << endl;
        }
        lineno_++;
    }
    ...
    std::string& errorMsg)
{
    ...
    if(is.fail())
    {
        errorMsg = "Unable to read hashchode";
        return NULL;
    }
